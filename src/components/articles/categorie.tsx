...
import { IonApp, IonButton, IonCol, IonList, IonModal, IonThumbnail, IonSearchbar, IonContent, IonAvatar, IonSelectOption, IonPage, IonItemDivider, IonSelect, IonRadioGroup, IonGrid, IonHeader, IonInput, IonItem, IonLabel, IonRow, IonTitle, IonToolbar, IonIcon, IonAlert, IonButtons, IonMenuButton, IonTabBar, IonTabButton, IonTabs, IonRouterOutlet, IonCard, IonCardContent, IonCardTitle, IonNote, IonBadge, IonRouterLink, IonCardSubtitle } from "@ionic/react";
import './categorie.css'
import { triangle, ellipse, square, arrowBack, arrowForward, personCircleOutline, globeOutline, calendar, informationCircle, map, personCircle } from 'ionicons/icons';
...
const Categories: React.SFC<Ajout_utiliformprops> = ({ Id, Panier }) => {
    const [idant, setIdant] = useState<any>(parseInt(window.location.pathname.split("/")[2]));
    ...
    const getartcat = () => {
        Axios.post('https://backend-shop.benindigital.com/articlecateg', {
            idcat: Id,
        }).then((ret) => {
            setArticle(ret.data);
        })
    ...
    const getcat = () => {
        Axios.post('https://backend-shop.benindigital.com/recupcat', {
            id: Id,
        }).then((ret) => {
    ...
    const getArrivage = () => {
        fetch('https://backend-shop.benindigital.com/arrivage').then((res) => {
            const data = res.json()
            return data
        }).then((data) => {
            setArticle(data);
    ...
    const getPop = () => {
        fetch('https://backend-shop.benindigital.com/populaire').then((res) => {
            const data = res.json()
            return data
        }).then((data) => {
            setArticle(data);
